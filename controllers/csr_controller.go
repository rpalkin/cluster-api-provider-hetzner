...
func getHbmmWithConstantHostname(ctx context.Context, csrUsername string, clusterName string, mCluster ManagementCluster) (*infrav1.HetznerBareMetalMachine, error) {
	...
	newProviderID := "hrobot://" + serverID
	oldProviderID := "hcloud://bm-" + serverID
	...
	for i := range hList.Items {
		...
		if *hList.Items[i].Spec.ProviderID == newProviderID || *hList.Items[i].Spec.ProviderID == oldProviderID {
			hbmm = &hList.Items[i]
			break
		}
	}
	if hbmm == nil {
		return nil, fmt.Errorf("ProviderID: %q or %q %w", newProviderID, oldProviderID, errNoHetznerBareMetalMachineByProviderIDFound)
	}
